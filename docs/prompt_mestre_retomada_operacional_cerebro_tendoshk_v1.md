# Prompt-Mestre de Retomada Operacional do Cérebro Tendoshk — V1
...
- deixar explícito quando a retomada está ocorrendo por evocação textual e quando está ocorrendo por verificação documental.
...
- a retomada correta deve respeitar bootstrap + checkpoint + registros de mudança + peças já estabilizadas;
...
### 5.2. Checkpoint
O checkpoint guarda o estado salvo da fase atual.
...
### 5.4. Ativação
A ativação em chat novo carrega bootstrap + checkpoint + verificação de mudanças, quando disponível.
...
Esses arquivos pertencem à camada estrutural do núcleo e não devem ser tratados como ruído periférico.
...
Usar quando a retomada, operação ou contexto depender de ambiente.

### 7.3. `ativos/`
Camada de artefatos canônicos integrais.

### 7.4. `ativos/contratos/`
Destino de documentos integrais canônicos de natureza:
- contratual
- protocolar
- normativa
- especificacional
...
Outros projetos podem ter seus próprios consolidados correspondentes.

### 7.6. `data/estado/`
Camada de estado técnico do NPT.
Arquivos atuais relevantes incluem:
- `consolidacoes.json`
- `status_npt.json`
- `ultima_revisao.json`

Tratar essa camada como estado técnico auxiliar do sistema.
...
Usar quando a retomada depender de exportações operacionais ou snapshot.
...
Tratar como camada de trânsito e insumo, não como destino canônico principal.
...
Tratar como apoio de rastreabilidade e não como peça narrativa principal.
...
Esses logs podem conter sinais importantes para fechamento e retomada.
...
Esses dois devem ser tratados como fluxo operacional principal do sistema.

### 7.12. `notebooks/legado_auxiliares/`
Camada de legado auxiliar.
Não confundir com fluxo principal atual.
...
### 9.1. Fonte soberana
Onde vivem regras e estados canônicos.

### 9.2. Fonte integral
Onde vivem documentos completos.

### 9.3. Fonte consolidada
Onde vive a memória viva sintética e cumulativa.

### 9.4. Fonte indexadora
Onde vivem índices e rastreabilidade.

### 9.5. Fonte versionada
Onde vivem espelhos versionados dos ativos integrais.
...
## 12. Regra de saída operacional herdada
Quando o modo de curadoria estrutural estiver ativo, o assistente não deve responder apenas com comentário, resumo genérico ou reflexão vaga.

A resposta deve produzir saída operacional.

Sempre que o contexto exigir curadoria estrutural, o assistente deve buscar:
- leitura do que o chat realmente contém;
- valor estrutural e impacto futuro;
- decisão de incorporação;
- projeto principal e, se couber, secundários;
- nome sugerido;
- descrição curta;
- decisão de indexação;
- `[NPT_ENTRY]`, se couber;
- `[CHAT_INDEX]`, se couber.
...
Essa mesma lógica deve valer, por analogia, para futuro repositório persistente autoral.
...
- lógica bootstrap + checkpoint + verificação de mudança;
...
- o estado salvo suficiente da fase;
- as travas vigentes;
- o que está estabilizado;
- o que não deve ser reaberto;
- o próximo passo natural, quando identificável;
- a verificação de mudanças desde o último checkpoint relevante, quando disponível.
...
- retomada por evocação textual
 e não em:
- retomada por verificação documental plena
...
Além do prompt, o assistente consulta bootstrap, checkpoint, peças relevantes e fontes acessíveis.
...
Se bootstrap, checkpoint ou outras fontes soberanas não estiverem acessíveis naquele chat:

1. carregar o regime pelo Prompt-Mestre de Retomada;
2. operar sob o mínimo confiável embutido no prompt;
3. explicitar que a retomada está ocorrendo por evocação textual e não por conferência plena das fontes;
...
2. verificar bootstrap e checkpoint;
3. verificar se houve mudanças posteriores;
4. distinguir se a dúvida é de regra permanente, estado atual, mudança, destino documental ou coerência;
...
- e, quando necessário, restaure explicitamente o próximo passo natural, o modo de verificação usado e se houve ou não houve mudança desde o último checkpoint relevante.