...
## 1. Natureza da peça
Este documento estabelece o protocolo formal de save state do Cérebro Tendoshk na fase V1.2.
...
Ela não substitui o Prompt Mestre de Curadoria Estrutural.
...
Na V1.2, um save state só é considerado válido quando preserva, no mínimo:
...
Um checkpoint rastreável da V1.2 deve conter, no mínimo:
...
A existência e implicação do checkpoint podem alimentar:
...
5. e, ao final de ciclo relevante, decidir se deve gerar novo checkpoint rastreável.
...
## 16. Separação dura de camadas operacionais
Esta peça deve respeitar, sem exceção, a seguinte separação:

### 16.1. Executar F5
Executar o F5 significa:
- identificar a linha de trabalho;
- buscar o último checkpoint relevante;
- verificar se existe checkpoint;
- verificar mudanças posteriores;
- classificar a continuidade;
- restaurar o próximo passo natural;
- e dizer se há ou não necessidade de gerar ou atualizar checkpoint.

### 16.2. Executar F5 NÃO significa
Executar o F5 **não significa**:
- gerar checkpoint automaticamente;
- gerar NPT_ENTRY automaticamente;
- gerar CHAT_INDEX automaticamente;
- ingerir no NPT automaticamente.

### 16.3. Fechar com F5
Fechar com F5 significa:
- depois do diagnóstico, gerar ou atualizar checkpoint **somente** quando o fechamento realmente exigir.

### 16.4. Ingestão no NPT
Ingestão no NPT é outra camada.
Ela só deve ocorrer quando houver decisão explícita de consolidar/ingerir.

### 16.5. Regra soberana crítica
**O F5, por padrão, não gera NPT_ENTRY.**
Mesmo quando o F5 concluir que um checkpoint deve ser criado, isso não autoriza, por si só, geração automática de NPT_ENTRY.

---

## 17. Relação com o Prompt-Mestre de Retomada
...
## 18. Relação com o Prompt Mestre de Curadoria V6.1
...
Se houver necessidade de gerar NPT_ENTRY, isso deve vir da camada de curadoria/ingestão ou de solicitação explícita de consolidação, e não como efeito automático de executar o F5.

---

## 19. Regra de fechamento com save state
...
## 20. Conteúdo mínimo do fechamento com F5
...
## 23. Fórmula operacional do F5
A fórmula operacional da V1.2 é:
...
8. operar
9. decidir se há ou não necessidade de gerar ou atualizar checkpoint

---

## 24. Critério de sucesso do protocolo
...
- usar o repositório oficial como trilha de busca versionada entre chats;
- e não atravessar para ingestão no NPT sem decisão explícita.

---

## 25. Cláusula de realidade operacional
...
## 26. Formulação canônica final
**O Protocolo F5 / Save State, Checkpoint Rastreável e Retomada Diferencial do Cérebro Tendoshk — V1.2 é a peça responsável por formalizar como o sistema salva o estado de uma linha de trabalho, reencontra esse estado de forma rastreável, busca prioritariamente o último checkpoint relevante no repositório oficial do cérebro quando ele estiver acessível, compara mudanças posteriores, classifica a continuidade e decide se há ou não necessidade de checkpoint, sem atravessar automaticamente para ingestão no NPT.**
...
- saber classificar a qualidade real da retomada;
- e saber que executar F5 não é o mesmo que ingerir no NPT.
